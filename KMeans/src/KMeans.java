...
// read image
    private void marchThroughImage(BufferedImage image) throws IOException {
     ...
     // for the first iteration
            ...
            for(int j=0;j<h*w;j++)
            {
                ...
                for(int i=0;i<k;i++)
                {
                    double distance=Math.sqrt(Math.pow((arr[i][0]- ((value[j] >> 16) & 0xff)),2)+
                            Math.pow((arr[i][1]- ((value[j] >> 8) & 0xff)),2)+ Math.pow((arr[i][2]- ((value[j]) & 0xff)),2));
       ...
       //     System.out.println(count);
        ...
        ImageIO.write(image, "jpg", new File("samiha2.bmp"));
    ...
    public KMeans() {
        try {
            ...
            BufferedImage image =
                    ImageIO.read(this.getClass().getResource("test.png"));
